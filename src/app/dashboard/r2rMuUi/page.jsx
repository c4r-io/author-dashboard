...
const Page = () => {
  ...
  const [r2rMuUiList, setR2rMuUiList] = useState({
    page: 1,
    pages: 1,
    r2rMuUis: null,
  });
  ...
  const getr2rMuUisList = async (page) => {
  ...
  useEffect(() => {
    getr2rMuUisList(page);
  }, [page]);
  const createSampleR2rMuUi = async () => {
    dispatchUserData({ type: 'checkLogin' });
    const config = {
      method: 'post',
      url: 'api/r2rMuUi',
      ...
      data: {
        ...
        headerContent: 'Sample Header content',
        headerFooter: 'Sample Header footer',
        wheelPreRunText1: 'Sample Wheel pre run text1',
        wheelPreRunText2: 'Sample Wheel pre run text2',
        wheelFooterToggleButtonText: 'Sample Wheel footer toggle button text',
        wheelDonePickingText: 'Sample Wheel done picking text',
        wheelDoneRechooseButtonText: 'Sample Wheel done rechoose button text',
        questionToUserText: 'Sample Question to user text',
        questionToUserButtonText: 'Sample Question to user button text',
        greatJobText: 'Sample Great job text',
        considerThisText: 'Sample Consider this text',
        closingScreenText: 'Sample Closing screen text',
        nextScreenButtonText: 'Sample Next screen button text',
      },
    };
    ...
    try {
      ...
      setLoading(false);
      router.push('/dashboard/r2rMuUi/' + response.data._id);
      toast.success('Sample R2r mu ui Created Successfully!', {
        position: 'top-center',
      });
    } catch (error) {
      if (error?.response?.status == 401) {
  ...
  const deleteR2rMuUi = async () => {
    dispatchUserData({ type: 'checkLogin' });
    const config = {
      method: 'delete',
      url: 'api/r2rMuUi/' + deleteId,
      headers: {
        Authorization: `Bearer ${getToken('token')}`,
      },
    };
    ...
    try {
      ...
      deleteFromList(r2rMuUiList, 'r2rMuUis', deleteId, setR2rMuUiList);
      if (r2rMuUiList.r2rMuUis.length < 5 && r2rMuUiList.pages !== 1) {
        getr2rMuUisList();
      }
      ...
      <div className={` ${loading ? '!pointer-events-none' : ''} `}>
        ...
        <div className="w-full flex justify-end pb-3">
          ...
          >
            ...
            {createLoading ? 'Creating...' : 'New R2r mu ui'}
          </button>
        </div>:''
        }
        <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
          <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            {!listLoading && (
              <tbody>
                {r2rMuUiList?.r2rMuUis?.map((item, index) => (
                  <tr
                    className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
                    key={index}
                  >
                    <td className="px-6 py-4">{item.headerTitle}</td>
                    <td className="px-6 py-4 text-right">
                      <div className="inline-flex space-x-1 items-center text-base font-semibold text-gray-900 dark:text-white">
                        <button
                          ...
                          onClick={() =>
                            router.push('/dashboard/r2rMuUi/' + item._id)
                          }
                        >
                          <svg
                            className="w-3 h-3 mr-1 text-white"
            ...
            <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
              ...
              <div className="p-6 text-center">
                ...
                <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                  Are you sure you want to delete this r2rMuUi?
                </h3>
                <button
                  data-modal-hide="popup-modal"
                  type="button"
                  className="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2"
                  onClick={() => deleteR2rMuUi()}
                >
                  Yes, I&apos;m sure
                </button>
                <button

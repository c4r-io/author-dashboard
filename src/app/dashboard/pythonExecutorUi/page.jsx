...
const Page = () => {
  ...
  const [pythonExecutorUiList, setPythonExecutorUiList] = useState({
    page: 1,
    pages: 1,
    pythonExecutorUis: null,
  });
  ...
  const getpythonExecutorUisList = async (page) => {
    ...
    try {
      ...
      setPythonExecutorUiList(response.data);
  ...
  const createSamplePythonExecutorUi = async () => {
    dispatchUserData({ type: 'checkLogin' });
    const config = {
      method: 'post',
      url: 'api/pythonExecutorUi',
    ...
    try {
      ...
      router.push('/dashboard/pythonExecutorUi/' + response.data._id);
      toast.success('Sample Python executor ui Created Successfully!', {
        position: 'top-center',
      });
    } catch (error) {
      if (error?.response?.status == 401) {
  ...
  const deletePythonExecutorUi = async () => {
    dispatchUserData({ type: 'checkLogin' });
    const config = {
      method: 'delete',
      url: 'api/pythonExecutorUi/' + deleteId,
      headers: {
        Authorization: `Bearer ${getToken('token')}`,
      },
    };
  ...
  useEffect(() => {
    getpythonExecutorUisList(page);
  }, [page]);
  return (
    <div className="container mx-auto py-4 px-4 md:px-0">
      <div>
        {pythonExecutorUiList?.pythonExecutorUis?.length < 1 ? (
          <div className="w-full flex justify-end pb-3">
            <button
              ...
              onClick={() => createSamplePythonExecutorUi()}
            ...
            >
              ...
              {createLoading ? 'Creating...' : 'New Python executor ui'}
            </button>
          </div>
        ) : (
          ''
        ...
        <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
          <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            {!listLoading && (
              <tbody>
                {pythonExecutorUiList?.pythonExecutorUis?.map((item, index) => (
                  <tr
                    className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
                    key={index}
                  >
                    <td className="px-6 py-4">
                      {getTrimedString(item.headerTitle)?.content}
                      {getTrimedString(item.headerTitle)?.isTrimed && '...'}
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="inline-flex space-x-1 items-center text-base font-semibold text-gray-900 dark:text-white">
                        <button
                          ...
                          onClick={() =>
                            router.push(
                              '/dashboard/pythonExecutorUi/' + item._id,
                            )
                          }
                        >
                          <svg
          ...
          {pythonExecutorUiList?.pythonExecutorUis?.length == 0 ? (
  ...
  dark:bg-gray-800"
        ...
        {pythonExecutorUiList.pages > 0 && (
          <Pagination
            activePage={page}
            pageLength={pythonExecutorUiList?.pages}
            ...
            <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
              ...
              <div className="p-6 text-center">
                ...
                <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                  Are you sure you want to delete this pythonExecutorUi?
                </h3>
                <button
                  data-modal-hide="popup-modal"
                  type="button"
                  className="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2"
                  onClick={() => deletePythonExecutorUi()}
                >
                  Yes, I&apos;m sure
                </button>
                <button
